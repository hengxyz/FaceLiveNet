### FaceLiveNet2.0 for Facial expression reocognition  #########
##
...
def main(args):
    ...
    ###########################   FER2013    ##########################
    # image_list, label_list, usage_list, nrof_classes, label_list_id = facenet_ext.get_image_paths_and_labels_fer2013(args.data_dir,
    ...
    if args.evaluate_express:
        ...
        ###########################   FER2013    ##########################
        # image_paths_test, label_list_test, usage_list_test, nrof_classes_test, label_list_id_test = facenet_ext.get_image_paths_and_labels_fer2013(args.data_dir,
        #                                                                                               args.labels_expression,
        #                                                                                               'PublicTest')
        ...
        # ###########################   SFEW    ##########################
        # image_paths_test, label_list_test, nrof_classses_test = facenet_ext.get_image_paths_and_labels_sfew(args.data_dir_test, args.labels_expression_test)
        ...
        for _ in range(nrof_preprocess_threads):
           ...
           # filenames, label_id, label_expr = input_queue.dequeue_up_to() ## tf0.12
            images = []
        ...
        logits_id = tf.identity(logits_id, 'logits_verif')
        ...
        learning_rate = tf.train.exponential_decay(learning_rate_placeholder, global_step,
                                                   ...
                                                   1.0, staircase=True)
        learning_rate_dyn = tf.train.exponential_decay(learning_rate_dyn_placeholder, global_step,
                                                   args.learning_rate_decay_epochs * args.epoch_size,
                                                   1.0, staircase=True)
        ...
        vars = update_gradient_vars_expr
            ...
            while epoch < args.max_nrof_epochs:
                ...
                learning_rate_ \
                    = train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op,
                            ...
                            best_authen_exprecog, epoch_current,loss_full, grads_full, grads_clip_full, learning_rate_dyn,
                            learning_rate_dyn_placeholder)
...
def train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op, image_paths_placeholder,
          labels_id_placeholder, labels_expr_placeholder,
          learning_rate_placeholder, phase_train_placeholder,
          batch_size_placeholder, global_step, loss_verif, loss_expr, summary_op,  summary_writer,
          regularization_losses, learning_rate_schedule_file, prelogits_center_loss_verif_n,
          cross_entropy_mean_verif, cross_entropy_mean_expr, acc, val, far, centers_cts_batch_reshape,
          logits_id, logits_expr, keep_probability_placeholder, update_gradient_vars_expr, acc_expression,
          each_expr_acc, label_batch_id, label_batch_expr, express_probs_confus_matrix, log_dir,
          model_dir, image_batch, learning_rate, phase_train_placeholder_expression,
    ...
    if args.learning_rate_dyn>0.0:
        lr_dyn = args.learning_rate_dyn
    else:
        lr_dyn = facenet_ext.get_learning_rate_from_file(learning_rate_schedule_file, epoch_current)

    print('Index_dequeue_op....')
    index_epoch = sess.run(index_dequeue_op)
    label_id_epoch = np.zeros(len(index_epoch))
    ...
    with open(os.path.join(log_dir, 'logits_weight_layer.txt'), 'at') as f:
        f.write('exp(logits_lossweights_embedings_0),  exp(logits_lossweights_embedings_0), exp(logits_lossweights_0), exp(logits_lossweights_1)\n')
    ...
    while batch_number < args.epoch_size:
        ...
        feed_dict = {learning_rate_placeholder: lr, learning_rate_dyn_placeholder: lr_dyn, phase_train_placeholder: False, phase_train_placeholder_expression: True, batch_size_placeholder: args.batch_size, keep_probability_placeholder: args.keep_probability}
        if (batch_number % 100 == 0):
            ...
            softmax_acc_expr_, cross_entropy_verif_, diff_mean_, centers_, _, loss_full_, grads_full_, \
            grads_clip_full_, learning_rate_dyn_, summary_str \
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            ...
                            cross_entropy_verif, diff_mean, centers, train_op_mainstem,  loss_full, grads_full,
                            grads_clip_full, learning_rate_dyn, summary_op], feed_dict=feed_dict)
            ...
            label_batch_expr_, image_batch_, learning_rate_, softmax_acc_verif_, \
            softmax_acc_expr_, cross_entropy_verif_, diff_mean_, centers_, _, loss_full_, grads_full_, \
            grads_clip_full_, learning_rate_dyn_ \
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            prelogits_center_loss_verif_n, cross_entropy_mean_verif, cross_entropy_mean_expr,
                            centers_cts_batch_reshape, logits_id, logits_expr, label_batch_id, label_batch_expr,
                            image_batch, learning_rate, softmax_acc_verif, softmax_acc_expr,
                            cross_entropy_verif, diff_mean,centers, train_op_mainstem, loss_full,grads_full,
                            grads_clip_full, learning_rate_dyn], feed_dict=feed_dict)
        print("step %d"%step)
        ...
        print('Epoch: [%d][%d][%d/%d]\tTime %.3f\n \
            ...
            (epoch, epoch_current, batch_number + 1, args.epoch_size, duration, loss_verif_, loss_expr_, cross_entropy_mean_verif_, \
             cross_entropy_mean_expr_, loss_full_, softmax_acc_verif_, \
